...
class QueryExpander:
    ...
    def find_candidate_expansion_terms(self, query, doc_freq_table, term_frequency_table):
        ...
        for q_term in query.lower().split():
            ...
            for term, docs in doc_freq_table.items():
                ...
                if term not in dice_coefficients:
                    dice_coefficients[term] = 2*len(q_docs.intersection(docs[0]))/(len(docs)+len(q_docs))/len(query.split()) 
                else:
                    dice_coefficients[term] += 2*len(q_docs.intersection(docs[0]))/(len(docs)+len(q_docs))/len(query.split())
